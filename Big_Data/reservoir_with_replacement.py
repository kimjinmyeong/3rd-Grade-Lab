...
# test reservoir sampling with replacement 10000 times.
for i in tqdm(range(10000)):
    # initialize @Reservoir object each time with @k = 1.
    reservoir_array = []
    for _ in range(100):
        reservoir_array.append(Reservoir(1))
    # put 0 ~ 999 interger stream in each @Reservoir object.
    for j in range(1000):
        for reservoir in reservoir_array:
            reservoir.put(j)
...
# plot @sample_count.
plt.title("reservoir_sampling with replacement")
